                        ...
                        try {
                            if (str_contains($sourceUrl, 'appweb.regione.vda.it')) {
                                try {
                                    ...
                                    Log::debug('Raw HTML content: ' . $itemContentCrawler->html());

                                    // Check if content exists
                                    $contentExists = $itemContentCrawler->filter('p')->count() > 0;
                                    Log::info('Found ' . $itemContentCrawler->filter('p')->count() . ' paragraphs');

                                    if (!$contentExists) {
                                        Log::error('No paragraphs found in the content');
                                        throw new \Exception('No content found');
                                    }

                                    // Get all text content from p tags, excluding the navigation elements
                                    $paragraphs = $itemContentCrawler->filter('p')->each(function ($node) {
                                        // Skip if this is a navigation paragraph (contains 'Indietro' link)
                                        if ($node->text() === 'Indietro' || str_contains($node->text(), 'javascript: history.go(-1)')) {
                                            return null;
                                        }
                                        return $node->text();
                                    });
                                    ...
                                    if (empty($paragraphs)) {
                                        Log::error('No valid paragraphs found after filtering');
                                        throw new \Exception('No valid content found');
                                    }

                                    // Get all bold text for potential titles
                                    $boldTexts = $itemContentCrawler->filter('b')->each(function ($node) {
                                        return trim($node->text());
                                    });

                                    Log::info('Found ' . count($boldTexts) . ' bold elements');

                                    // Get the first meaningful bold text as title
                                    $title = null;
                                    foreach ($boldTexts as $boldText) {
                                        if (strlen($boldText) > 5) { // Ensure it's a meaningful title
                                            $title = $boldText;
                                            break;
                                        }
                                    }
                                    ...
                                    if (!$title) {
                                        // Fallback: use the first sentence of content as title
                                        $firstParagraph = reset($paragraphs);
                                        $title = substr($firstParagraph, 0, strpos($firstParagraph, '.') ?: 100);
                                        Log::warning('No bold title found, using fallback: ' . $title);
                                    }

                                    // Combine paragraphs for description
                                    $description = implode("\n\n", $paragraphs);

                                    // Clean up the content
                                    $description = preg_replace('/\s+/', ' ', $description); // normalize whitespace
                                    $description = trim($description); // remove leading/trailing whitespace

                                    if ($title && $description) {
                                        Log::info('Successfully extracted content:');
                                        Log::info('Title length: ' . strlen($title));
                                        Log::info('Description length: ' . strlen($description));
                                    } else {
                                        throw new \Exception('Failed to extract title or description');
                                    }
                                    ...
                                    Log::error('Error processing appweb.regione.vda.it content: ' . $e->getMessage());
                                    Log::error('URL being processed: ' . $sourceUrl);
                                    throw $e;
                                }
                            }
