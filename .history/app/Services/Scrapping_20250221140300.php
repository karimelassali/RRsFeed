                        ...
                        try {
                            if (str_contains($sourceUrl, 'appweb.regione.vda.it')) {
                                try {
                                    ...
                                    $mainContentDiv = $itemContentCrawler->filter('.noborder')->first();

                                    if ($mainContentDiv->count() === 0) {
                                        Log::error('No .noborder div found');
                                        continue;
                                    }

                                    // Get all paragraphs within the .noborder div
                                    $mainContent = $mainContentDiv->filter('p')->first();

                                    if ($mainContent->count() === 0) {
                                        Log::error('No paragraphs found in .noborder div');
                                        continue;
                                    }

                                    // Extract title (first bold text)
                                    $title = $mainContent->filter('.testi')->first()->text();

                                    // Extract description (all text from the first p tag)
                                    ...
                                    if (empty($title) || empty($description)) {
                                        Log::error('Empty title or description extracted');
                                        continue;
                                    }

                                    Log::info('Successfully extracted content from .noborder div');
