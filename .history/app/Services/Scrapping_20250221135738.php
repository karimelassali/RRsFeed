                    ...
                    try {
                        ...
                        try {
                            if (str_contains($sourceUrl, 'appweb.regione.vda.it')) {
                                try {
                                    // First, let's get the title from .testi div
                                    $title = null;
                                    if ($itemContentCrawler->filter('.testi')->count() > 0) {
                                        $title = trim($itemContentCrawler->filter('.testi')->text());
                                        Log::info('Found title in .testi: ' . $title);
                                    }

                                    if (!$title) {
                                        Log::warning('No title found in .testi div');
                                        continue;
                                    }

                                    // For description, we need to:
                                    // 1. Get all content from relevant paragraph tags
                                    // 2. Filter out JavaScript and other unwanted content
                                    $description = '';

                                    $itemContentCrawler->filter('p')->each(function ($node) use (&$description) {
                                        $text = $node->text();

                                        // Skip if the text contains JavaScript markers
                                    ...
                                    $description = preg_replace('/function\s+\w+\s*\([^)]*\)\s*{[^}]*}/i', '', $description);

                                    if (empty($description)) {
                                        Log::warning('No valid description content found');
                                        continue;
                                    }
                                    ...
                                    if (strlen($description) < 50) { // Arbitrary minimum length
                                        Log::warning('Description too short, might be invalid');
                                        continue;
                                    }

                                    if (str_contains($description, 'function') || str_contains($description, 'var ')) {
                                        Log::warning('Description still contains code-like content, skipping');
                                        continue;
                                    }
