                    ...
                    try {
                        ...
                        try {
                            if (str_contains($sourceUrl, 'appweb.regione.vda.it')) {
                                try {
                                    // Get the first p tag that contains the main content
                                    $mainParagraph = $itemContentCrawler->filter('p')->first();

                                    // Extract title: First bold text in the content
                                    $title = $mainParagraph->filter('.testi')->first()->text();

                                    // Extract description: Combine all text content, preserving quotes and formatting
                                    $description = $mainParagraph->each(function ($node) {
                                        return $node->text();
                                    })[0];
                                    ...
                                    if ($title && $description) {
                                        Log::info('Successfully extracted content from appweb.regione.vda.it');
                                        Log::debug('Title: ' . $title);
                                        Log::debug('Description length: ' . strlen($description));
                                    } else {
                                        Log::error('Failed to extract title or description from appweb.regione.vda.it');
                                        throw new \Exception('Missing title or description');
                                    }
                                } catch (\Exception $e) {
                                    Log::error('Error extracting content from appweb.regione.vda.it: ' . $e->getMessage());
                                    Log::debug('HTML structure: ' . $itemContentCrawler->html());
                                    throw $e;
                                }
                            }
