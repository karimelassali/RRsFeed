            ...
            try {
                ...
                foreach ($crawler->filter('item') as $item) {
                    try {
                        ...
                        try {
                            // First, modify the extraction function to return title and description directly:
private function extractContent($crawler, $selector)
{
    try {
        ...
        Log::error("Error in extractContent: " . $e->getMessage());
        return null;
    }
}

// Then in the processRssFeeds function, modify the appweb.regione.vda.it section:
if (str_contains($sourceUrl, 'appweb.regione.vda.it')) {
    try {
        ...
        $paragraphs = $itemContentCrawler->filter('p');
        Log::info('Found ' . $paragraphs->count() . ' paragraphs');
        ...
        // Extract main content paragraph
        $mainContent = '';
        $paragraphs->each(function ($node) use (&$mainContent) {
            // Skip navigation paragraphs
            if (!str_contains($node->text(), 'Indietro') &&
                !str_contains($node->text(), 'javascript:')) {
                $mainContent .= $node->text() . "\n";
            }
        });

        // Find the first significant bold text for title
        $title = '';
        $itemContentCrawler->filter('b')->each(function ($node) use (&$title) {
            if (empty($title) && strlen(trim($node->text())) > 5) {
                $title = trim($node->text());
            }
        });

        if (empty($title)) {
            Log::warning('No suitable title found in bold text');
            continue;
        }

        if (empty($mainContent)) {
            Log::warning('No suitable content found');
            continue;
        }

        // Clean up the content
        $description = trim(preg_replace('/\s+/', ' ', $mainContent));

        // Set these directly instead of returning an array
        $title = $title;
        $description = $description;

        Log::info('Extracted content successfully');
        Log::debug('Title: ' . $title);
        Log::debug('Description length: ' . strlen($description));
