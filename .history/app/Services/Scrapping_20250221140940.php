                ...
                foreach ($crawler->filter('item') as $item) {
                    try {
                        ...
                        try {
                            ...
                            //     try {
                            //         // Find the first meaningful paragraph (excludes navigation)
                            //         $mainParagraph = null;

                            //         $itemContentCrawler->filter('p')->each(function ($node) use (&$mainParagraph) {
                            //             // Skip if this is a navigation paragraph or empty
                            //             if (str_contains($node->text(), 'Indietro') ||
                            //                 str_contains($node->text(), 'FINE CONTENTGC') ||
                            //                 trim($node->text()) === '') {
                            //                 return;
                            //             }

                            //             // If we haven't found our main paragraph yet and this one has content
                            //             if ($mainParagraph === null && strlen($node->text()) > 100) {
                            //                 $mainParagraph = $node;
                            ...
                            //         if (!$mainParagraph) {
                            //             Log::error('Main content paragraph not found');
                            //             return null;
                            //         }

                            //         // Get the first significant bold text as title (that isn't a link or social media text)
                            //         $title = null;
                            //         $mainParagraph->filter('b')->each(function ($node) use (&$title) {
                            //             $text = trim($node->text());
                            //             // Skip if it's a link, social media, or too short
                            //             if (!str_contains($text, 'FOTO') &&
                            ...
                            //         // Get the full text for description
                            //         $description = '';
                            //         $mainParagraph->filter('font')->each(function ($node) use (&$description) {
                            //             $text = trim($node->text());
                            //             if ($text !== '') {
                            //                 $description .= $text . ' ';
                            //             }
                            ...
                            //         // Remove any social media handles and links
                            //         $description = preg_replace('/@\w+/', '', $description);
                            //         $description = preg_replace('/https?:\/\/\S+/', '', $description);

                            //         // Remove any remaining navigation text
                            //         $description = str_replace('Indietro', '', $description);

                            //         if (empty($title) || empty($description)) {
                            //             Log::error('Failed to extract title or description');
                            //             return null;
                            //         }
                            ...
                            //     }
                            // }
                            eif (str_contains($sourceUrl, 'ansa.it')) {
